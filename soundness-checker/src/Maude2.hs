...
data Feature = F
  ...
  , _childGroups :: M.Map GroupID Group
  }
...
isWellFormed ft fid = all isWellFormed' (M.assocs ft)
  where
     ...
     gs f = maybe mempty (\pf -> M.elems $ M.filter (\g -> fid `elem` (_childFeatures g)) $ _childGroups pf) $ M.lookup (fromJust $ _parentID f) ft
...
addFeature (FM rfid ft) (newFid, newName, targetGroup, fType)
 ...
 where
   ...
   ft'' = M.insert newFid (F {_parentID = Just parentFid, _name = newName, _featureType = fType, _childGroups= mempty }) ft'
...
addFeatureToGroup ft targetGroup newFid = M.adjust (over childGroups (fmap (\g -> if targetGroup == _groupID g then over (childFeatures) (S.insert newFid) g else g))) parentID ft
  where
    parentID = parentOfGroup ft targetGroup

parentOfGroup :: FT -> GroupID -> FeatureID
parentOfGroup ft gid = fromJust $ M.foldrWithKey (\k f acc -> if gid `elem` M.keysSet (_childGroups f) then Just k else acc) (error $ "no parent found for group " ++ show gid) ft
...
parentGroup ft fid = not . null $ M.foldr (\f acc -> acc `mappend` M.filter (\g -> fid `elem` (_childFeatures g)) (_childGroups f) ) mempty ft

removeFeatureFromParent :: FT -> FeatureID -> FeatureID -> FT
removeFeatureFromParent ft parentFid fid = M.adjust (over childGroups (\gs -> M.map (over childFeatures (S.filter (/= fid))) gs)) parentFid ft
...
addGroup (FM rfid ft) (fid, gid, gtype)
  | isJust fj && isUniqueGroupID gid ft -- Maude seems fishy here, reduntant update of FT?
    = FM rfid $ M.adjust (over childGroups (M.insert gid (G {_groupID = gid, _groupType = gtype, _childFeatures= mempty}))) fid ft
...
removeGroup (FM rfid ft) gid
  | isJust fidj && (all (null . _childFeatures) . M.elems . snd3 . fromJust) fidj = FM rfid $ M.adjust (over childGroups (const gs)) fid ft
  | otherwise = error "removeGroup"
  where
     fidj = M.foldrWithKey (\fk f acc -> let (g,gs) = M.partition (\gx -> _groupID gx == gid) (_childGroups f) in if (not . null) g then Just (fk, g, gs) else acc) Nothing ft
...
changeGroupVariationType (FM rfid ft) (gid, gtype)
  ...
  where
     ...
     gs   = thd3 . fromJust $ fidj
     fs   = _childFeatures . head . M.elems . snd3 . fromJust $ fidj -- XXX
     -- couldn't be bothered to do the 2nd `over`
     ft'' = M.adjust (over childGroups (const (M.insert gid (G {_groupID=gid, _groupType=gtype, _childFeatures = fs}) gs))) parentFid ft

allWellFormed :: Set FeatureID -> FT -> Bool
...
moveGroup (FM rfid ft) (gid, newParent)
  ...
  where
    ...
    oldParentJ = M.foldrWithKey (\fk f acc -> let (g,gs) = M.partition (\gx -> _groupID gx == gid) (_childGroups f) in if (not . null) g then Just ((fk,f), g, gs) else acc) Nothing ft'
    oldParent = fst . fst3 . fromJust $ oldParentJ
    oldParentf = snd . fst3 . fromJust $ oldParentJ
    gs' = thd3 . fromJust $ oldParentJ
    g = head . M.elems. snd3 . fromJust $ oldParentJ
    fs = _childFeatures g
    ft'' = M.delete oldParent ft'
    ft''' = M.insert newParent (over childGroups (M.insert (_groupID g) g) f) $ M.insert oldParent (over childGroups (const gs') oldParentf) ft''
    ft'''' = updateParents ft''' fs newParent

allNotSubFeature :: Set FeatureID -> FeatureID -> FeatureID -> FT -> Bool
-- NB: negation vs. original `all'.
allNotSubFeature fs fid rfid ft = S.null $ S.filter (\f -> isSubFeature f fid rfid ft) fs

updateParents :: FT -> Set FeatureID -> FeatureID -> FT
